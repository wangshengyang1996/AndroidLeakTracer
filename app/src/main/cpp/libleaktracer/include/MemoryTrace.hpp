...
// (creates one if called for the first time)
    inline MemoryTrace::ThreadMonitoringOptions &MemoryTrace::getThreadOptions(void) {
        ThreadMonitoringOptions *pOpt = reinterpret_cast<ThreadMonitoringOptions *>(pthread_getspecific(
                __thread_options_key));
        if (pOpt == NULL) {
...
// monitoring for all threads
    inline void MemoryTrace::stopMonitoringPerThreadAllocations(void) {
        ...
        MutexLock lock(__threadListMutex);
        for (list_monitoring_options_t::iterator it = __listThreadOptions.begin();
             it != __listThreadOptions.end(); ++it) {
...
//            Dl_info dl_info = {0};
//            dladdr(info->allocStack[0], &dl_info);
//            __android_log_print(ANDROID_LOG_INFO, "wsy",
//                                "info->allocStack[0] = %ld,dlInfo: fname = %s\r\nsname = %s\r\n,fbase = %ld,\r\nsaddr = %ld",
//                                info->allocStack[0],dl_info.dli_fname, dl_info.dli_sname,
//                                dl_info.dli_fbase,dl_info.dli_saddr);
        }
